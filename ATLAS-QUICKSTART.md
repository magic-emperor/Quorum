...
```bash
cp ATLAS-CLAUDE/atlas.config.json.template /your/project/atlas.config.json
```

That's it. No npm install. No build step. ATLAS is markdown files that Claude Code reads.
...
```
/atlas-new "Build a todo API with user authentication"
```
...
Type `APPROVE` — ATLAS builds directly (SIMPLE project, skips architecture review).
...
```
/atlas-new "Build a SaaS for team project management with roles, billing, and real-time updates"
```

ATLAS classifies this as **COMPLEX** and runs the full pipeline:
...
Review `.atlas/context/architecture-proposal.md` if you want full detail. Type `APPROVE`.
...
Review `.atlas/BUGS.md` if you want detail. Type `APPROVE`. Done.
...
ATLAS reads `.atlas/` — loads full context automatically:
...
└── atlas.config.json            ← your ATLAS configuration
```

Commit `.atlas/` to Git. It's part of your project history.
...
Yes. Type `/stop` at any time. ATLAS saves exact state. Resume with:
```
/atlas-enhance continue current task
```

**Can I skip phases?**
Yes. For simple changes use `/atlas-enhance` (skips architecture + design phases).
To skip scaling analysis: set `"prompt_scaling_phase_6": false` in `atlas.config.json`.

**What if an agent makes a wrong decision?**
Type your correction during any checkpoint. ATLAS classifies it and updates the plan.
Or use `/atlas-rollback` to return to any previous checkpoint.

**Does this replace my existing Claude Code agents?**
No. ATLAS calls them. `planner`, `architect`, `tdd-guide`, `e2e-runner`, `security-reviewer`
all still work exactly as before. ATLAS adds orchestration and memory on top.
...
```
/atlas-new "description"        Start new feature or project
/atlas-enhance "change"         Modify existing feature (reads context automatically)
/atlas-status                   Current phase, decisions, token cost
/atlas-rollback                 Return to previous checkpoint
