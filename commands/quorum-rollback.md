---
description: Return the project to a previous rollback point created by QUORUM. Lists available rollback points, confirms selection, then restores. Commits new restore point so rollback can itself be rolled back.
---

# /quorum-rollback

Return your project to a previous state that QUORUM saved automatically.
...
/quorum-rollback --to rp_001_architecture_approved
```

## Rollback Points QUORUM Creates Automatically
...
```
QUORUM ROLLBACK — Available Restore Points
...
1. Shows what will change:
   ...
   .quorum/ state: restored to session [ID]
...
4. Restores `.quorum/` to the snapshot from that point
...
- `.quorum/plan.md` state from that point
- `.quorum/nervous-system/` from that point

**NOT restored:**
- `.quorum/history_plan.md` — always append-only, never reverted
- `BUGS.md` — bugs found are always preserved, even through rollbacks
- `bug-registry.json` — bug knowledge is permanent

## Related Commands

- `/quorum-status` — see current state before deciding to rollback
- `/quorum-sync` — re-index after major manual changes instead of rolling back
