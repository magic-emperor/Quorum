---
description: Force re-index of entire project. Updates project-index.json and function-registry.json. Use after major manual changes outside QUORUM, after large merges, or when agents seem to not know about recent code changes.
---

# /quorum-sync

Force re-index the project so QUORUM agents have accurate knowledge of the current codebase.

## Usage

```
/quorum-sync
/quorum-sync --functions-only
/quorum-sync --report
```

## When to Use

- After manually editing code outside QUORUM (the index may be stale)
...
> **Note:** Incremental updates via git hooks handle routine changes automatically. Use `/quorum-sync` only when the index is genuinely stale.
