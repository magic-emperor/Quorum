---
name: atlas-loop-prevention
description: Rules for all ATLAS confidence loops. Read before any agent enters a confidence loop with another agent. Extends loop-operator with ATLAS-specific escalation triggers, budgets, and hallucination prevention.
---
...
All ATLAS loops use loop-operator as their base layer.

## Foundation

All loop-operator rules apply to every ATLAS loop.
This skill adds ATLAS-specific escalation triggers on top.

## ATLAS-Specific Escalation Triggers
...
Format:
  "ATLAS CONFIDENCE LOOP ESCALATION
...
Mitigation:
`atlas-critic` runs alongside every loop.
