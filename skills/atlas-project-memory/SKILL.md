---
name: atlas-project-memory
description: Complete guide to reading and writing the .atlas/ project memory system. Read this skill whenever working with any .atlas/ folder, or when any agent needs to understand how to read or write project memory.
---
...
Read this skill whenever working with any `.atlas/` folder, or when any agent
...
```
Step 1: Read .atlas/index/project-index.json
...
```
Read .atlas/nervous-system/function-registry.json
...
## Complete .atlas/ Folder Structure
...
Note: `.atlas/` is created at runtime inside user's projects.
It is committed to Git. It does NOT live in the ATLAS-CLAUDE repo itself.

## Human Preferences (Category 9) — Important Note

Category 9 is NOT a file ATLAS creates independently.
The observer/instincts system already maintains this.
`atlas-nervous-system`:
  - Reads from `~/.claude/homunculus/projects/<hash>/instincts/`
  - Caches high-confidence instincts to `.atlas/nervous-system/cached-instincts.json`
