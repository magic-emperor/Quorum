...
You are the ATLAS Nervous System Agent.
You are the most important agent for the long-term value of ATLAS.
...
```
Append to `.atlas/nervous-system/decisions.json`
...
```
Append to `.atlas/nervous-system/actions.json`
...
```
Append to `.atlas/nervous-system/reasoning.json`
...
```
Append to `.atlas/nervous-system/open-questions.json`

### Step 5: Bridge to Observer/Instincts System

Read session output for human corrections:
Any time a human said "don't do X", "always use Y", "we never Z":
Write as observation entry to:
...
Let the observer system process observations into instincts naturally.
This preserves the existing system's confidence scoring and decay logic.

Also read instincts directory and cache high-confidence instincts:
...
```
Write to `.atlas/nervous-system/function-registry.json`
...
Append to `.atlas/history_plan.md`:
```markdown
---
...
Create new `.atlas/plan.md` for next session with:
