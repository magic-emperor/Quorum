...
import type { QUORUMRunOptions, RoutingTable, MapResult } from '../types.js'
...
- [issue]: [location, impact]`,
  ...
  onAgentOutput?.('quorum-backend-architect (map)', mapResponse.content)
  ...
  // This is the headline output of quorum map: a dev guide ready to commit.
  const devGuideFile = path.join(quorumDir, 'DEVGUIDE.md')
  const devGuide = `# Developer Guide
Generated by \`quorum map\` on ${new Date().toISOString()}
Codebase area: ${area} | Files scanned: ${files.length}

> This file is auto-generated. Run \`quorum map\` again after major refactors.
...
`
  ...
  const contextDir = path.join(quorumDir, 'context')
...
`
  ...
  onProgress?.(``)
  onProgress?.(`✓ DEVGUIDE.md written to .quorum/DEVGUIDE.md`)
  onProgress?.(`  Commit this file — new team members can read it to onboard instantly.`)
  onProgress?.(`  Also saved to .quorum/context/codebase-map.md for internal context.`)
  onProgress?.(`  Run quorum enhance to build on this existing codebase.`)

  return {
    area,
    files_scanned: files.length,
    modules_found: moduleLines,
    architecture_summary: 'See .quorum/DEVGUIDE.md',
    key_patterns: [],
